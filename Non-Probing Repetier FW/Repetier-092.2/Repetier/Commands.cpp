...
void motorCurrentControlInit() //Initialize Digipot Motor Current
{
    ...
    const uint8_t pwm_motor_current[] = DEFAULT_PWM_MOTOR_CURRENT;
...
void Commands::processGCode(GCode *com)
{
...
#else
        //GCode::executeFString(Com::tZProbeStartScript); //this did GCode from configuration.h as a G1 move.. too fast
        Printer::moveTo(IGNORE_COORDINATE,IGNORE_COORDINATE,100,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
         //^^^ starts probing at Z100, but at speed definable in eeprom
    ...
    break;
#endif
    case 68: // G68 2 point horizontal radius
    {
#if DISTORTION_CORRECTION
        float oldFeedrate = Printer::feedrate;
        Printer::measureDistortion();
        ...
        bool oldAutolevel = Printer::isAutolevelActive();
        Printer::setAutolevelActive(false);
        float sum = 0, last, hradius,oldFeedrate = Printer::feedrate;
