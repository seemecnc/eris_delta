...
void Printer::homeZAxis() // Delta z homing
{
    ...
    if(ENDSTOP_Z_BACK_ON_HOME > 0)
        PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerMM[Z_AXIS] * -ENDSTOP_Z_BACK_ON_HOME * Z_HOME_DIR,0,EEPROM::zProbeXYSpeed(), true, false);
#endif
    ...
    PrintLine::moveRelativeDistanceInSteps(0,0,dm,0,3,true,false);
    currentPositionSteps[X_AXIS] = 0;
    currentPositionSteps[Y_AXIS] = 0;
    ...
    transformCartesianStepsToDeltaSteps(currentPositionSteps, currentDeltaPositionSteps); //SeeMeCNC This damn thing causes twitch after setting endstop offsets
...
void Printer::homeAxis(bool xaxis,bool yaxis,bool zaxis) // Delta homing code
{
    ...
    moveToReal(0,0,Printer::zLength,IGNORE_COORDINATE,3);
...
float Printer::runZProbe(bool first,bool last,uint8_t repeat,bool runStartScript)
{
    ...
    if(first)
    {
        ...
        if(currentPosition[Z_AXIS] > EEPROM::zProbeBedDistance()) {
            ...
            moveTo(IGNORE_COORDINATE,IGNORE_COORDINATE,EEPROM::zProbeBedDistance(),IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
        }
